...
namespace Gym_Management_System
{
    public partial class Edit_Member : Form
    {
        ...
        private void btn_save_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string query = "Update NeMember set Fname = '" + txt_fname.Text + "' , Gender = '" + gender +"', Dob = '" + dob_picker.Value + "' , Mobile = '" + txt_mobile.Text + "' , Email = '" + txt_email.Text + "' , MembershipTime = '" + cmb_mTime.Text + "' , JoinDate = '" + join_picker.Value + "' , GymTime = '" + cmb_gymTime.Text + "' , Address = '" + txt_address.Text + "' where MemberId = '" + txt_mid.Text + "'";
        ...
        private void btn_reset_Click(object sender, EventArgs e)
        {
            ...
            if (dialog == DialogResult.Yes)
            {
                try
                {
                    string query = "Delete from NeMember where MemberId = '" + txt_mid.Text + "'";
                    int i = nm.save_update_delete(query);
                    if (i == 1)
